...
-- Hook the button's click rather than raw-replacing AbandonQuest().
-- Raw replacement of AbandonQuest can taint the call chain and may read
-- a stale GetAbandonQuestName() if another addon (e.g. Leatrix) has
-- changed the quest log selection via SelectQuestLogEntry between the
-- button click and the confirmation popup's OK being pressed.
...
-- Use hooksecurefunc instead of raw replacement to avoid tainting the
-- QuestLog_Update call chain (which controls QuestLogAbandonButton visibility).
