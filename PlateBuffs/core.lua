...
----------------------------------------------
	-- Claude: /pb debug toggles per-event combatlog tracing
	if input and input:lower():match("^%s*debug%s*$") then
		core.PB_DEBUG = not core.PB_DEBUG
		core.DEBUG = core.PB_DEBUG -- Claude: enable underlying core.Debug emitter too
		DEFAULT_CHAT_FRAME:AddMessage("|cff33ff99PlateBuffs|r debug = "..tostring(core.PB_DEBUG))
		return
	end
