...
local CONTACT_BUTTON_SIZE = 36   -- must match ContactContainer.lua
local CONTACT_BUTTON_MARGIN = 12
local CONTAINER_PADDING = 4     -- must match ContactContainer.lua
...
local function CreateContactButton(index)
    ...
    if (not button) then
        button = CreateFrame("CheckButton", CONTACT_BUTTON .. index, ADDON.contactContainer, "PopupButtonTemplate")
    end

    -- Explicitly set size: PopupButtonTemplate size may differ between client versions
    -- SetSize(w,h) doesn't exist in 3.3.5; use SetWidth/SetHeight separately
    button:SetWidth(CONTACT_BUTTON_SIZE)
...
local function SetContactButtonPosition(button)
    ...
    if (index == 1) then
        button:SetPoint("TOPLEFT", ADDON.contactContainer, "TOPLEFT", CONTAINER_PADDING, -CONTAINER_PADDING)
