...
local radioButtonCounter = 0
local function BuildRadio(parentFrame, text, relativeTo)
    radioButtonCounter = radioButtonCounter + 1
    local buttonName = "FavoriteContactsRadioButton" .. radioButtonCounter
    local button = CreateFrame("CheckButton", buttonName, parentFrame, "UIRadioButtonTemplate")
    button:SetPoint("LEFT", relativeTo, "RIGHT", 15, 0)
    -- In 3.3.5, the FontString child is named $parentText, accessible via _G[name.."Text"]
...
local function BuildSlider(frame, text, yOffset)
    ...
    local sliderName = "FavoriteContactsSlider" .. sliderCounter
    local slider = CreateFrame("Slider", sliderName, frame, "OptionsSliderTemplate")
    slider:SetPoint("LEFT", label, "RIGHT", 15, 0)
    slider:SetWidth(250)
    slider:SetValueStep(1)
    -- SetStepsPerPage does not exist in 3.3.5; SetObeyStepOnDrag guarded below
    slider:SetMinMaxValues(1, 20)
    if slider.SetObeyStepOnDrag then slider:SetObeyStepOnDrag(true) end
    -- In 3.3.5, OptionsSliderTemplate children are named $parentLow/$parentHigh/$parentText
...
local function BuildFrame()
    ...
    frame.scaleEditManual:SetWidth(183 - frame.scaleRadioAuto.text:GetWidth() - frame.scaleRadioManual.text:GetWidth())
    frame.scaleEditManual:SetHeight(20)
