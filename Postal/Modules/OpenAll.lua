...
updateFrame:SetScript("OnEvent", function(self, event, ...)
	if event == "MAIL_INBOX_UPDATE" then
		if PendingDel then
			PendingDel = false
			-- Accelerate the timer to fire very soon; avoids calling TakeInboxItem inside an event handler
			self.time = 0.05
		elseif wait then
			-- Server signalled; accelerate the timer instead of calling ProcessNext directly.
			-- Calling TakeInboxItem inside an event handler can cause the request to be silently dropped.
			self.time = 0.05
		end
	end
end)
...
refreshFrame:SetScript("OnHide", function(self)
	self:UnregisterEvent("MAIL_INBOX_UPDATE")
end)
refreshFrame:SetScript("OnEvent", function(self, event, ...)
	if event == "MAIL_INBOX_UPDATE" and self.mode == nil then
		-- Server has responded to CheckInbox(); now the counts are valid
		local current, total = GetInboxNumItems()
		if current == 50 or current == total then
			-- Full batch received or all remaining mail is now shown
			self.time = 1
			self.mode = 1
		else
			-- Unexpected partial result, retry soon
			self.time = 2
		end
	end
end)
refreshFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	if self.time <= 0 then
		if self.mode == nil then
			-- Send CheckInbox request; wait for MAIL_INBOX_UPDATE to read results
			self.time = 1  -- Claude: reduced from 8s; fallback timeout if MAIL_INBOX_UPDATE never arrives
	...
	if mailIndex > 0 then
		...
		if wait then
			...
			elseif lastNumAttach ~= attachCount then
				...
				-- Nothing has changed; track how long we have been stuck
				local now = GetTime()
				if not stuckTime then
					stuckTime = now
				elseif now - stuckTime > 1 then -- Claude: reduced from 5s
					-- Stuck for 1+ seconds - skip this mail and keep going
					Postal:Print(format("Postal: message %d stuck, skipping.", mailIndex))
					stuckTime = nil
					wait = false
					skipFlag = true
					mailIndex = mailIndex - 1
