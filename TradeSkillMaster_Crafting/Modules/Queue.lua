...
function Queue:HasLoop(itemString, steps, visited)
	...
	if not TSM.db.global.neverCraftInks or not TSMAPI.InkConversions[itemString] then
		local spellID = FindIntermediateSpellID(itemString)
		if spellID and TSM.db.factionrealm.crafts[spellID] then
...
function Queue:GetQueue()
	...
	-- inventoryTotals already initialized above
