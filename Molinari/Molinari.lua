...
local function ScanTooltip(self, spells)
    for index = 1, self:NumLines() do
        local lineText = _G['GameTooltipTextLeft' .. index]:GetText()
        if lineText and spells[lineText] then
            return unpack(spells[lineText])
        end
    end
end
...
function button:PLAYER_LOGIN()
    ...
    if(IsSpellKnown(51005)) then
        spells[ITEM_MILLABLE] = {GetSpellInfo(51005), 1/2, 1, 1/2} -- Milling
    end

    if(IsSpellKnown(31252)) then
        spells[ITEM_PROSPECTABLE] = {GetSpellInfo(31252), 1, 1/3, 1/3} -- Prospecting
    end

    if(IsSpellKnown(1804)) then
        spells[LOCKED] = {GetSpellInfo(1804), 1, 0.5, 0} -- Lockpicking, using LOCKED instead of ITEM_LOCKED
    end

    -- I wish Blizzard could treat disenchanting the same way
    if(IsSpellKnown(13262)) then
        disenchanter = {GetSpellInfo(13262), 1, 1, 1} -- Disenchanting
    end
