...
local function GetTSMAddon()
	-- TSM is a local variable inside TradeSkillMaster.lua, not a global.
	-- Retrieve it via AceAddon's registry instead.
	if LibStub then
		local AceAddon = LibStub( "AceAddon-3.0", true )
		if AceAddon then
			return AceAddon:GetAddon( "TradeSkillMaster", true )
		end
	end
	return nil
end

local function AuxItemKey( h )
	-- item link: |Hitem:itemID:enchant:gem1:gem2:gem3:gem4:suffixID:uniqueID|h
	-- suffix is non-zero for random-stat items (e.g. greens "of the Bear")
	local itemID, suffixID = h:match( "item:(%d+):%d+:%d+:%d+:%d+:%d+:(%-?%d+):?" )
	if not itemID then return nil end
	return itemID .. ":" .. ( suffixID or "0" )
end

function Rule.Execute.auxovervendor( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "auxovervendor"
	...
	local pct = arg1 or 0  -- default: aux simply exceeds vendor (0% over)
	local multiplier = 1 + ( pct / 100 )

	-- get vendor sell price
	local vendor_per = select( 11, GetItemInfo( Rule.Item.h ) )
	if not vendor_per or vendor_per <= 0 then
		return false
	end
	local vendor_price = vendor_per * ( Rule.Item.count or 1 )

	-- get aux market price
	local aux_price = nil
	if TSMAPI and TSMAPI.GetItemValue then
		aux_price = TSMAPI:GetItemValue( Rule.Item.h, "AuxMarket" )
	end
	if not aux_price or aux_price == 0 then
		local itemKey = AuxItemKey( Rule.Item.h )
		if itemKey and aux and aux.faction then
			for _, factionData in pairs( aux.faction ) do
				if factionData.history then
					local raw = factionData.history[itemKey]
					if raw then
						local daily_min = raw:match( "^[^#]*#([^#]*)" )
						local price = tonumber( daily_min )
	...
	return aux_price >= ( vendor_price * multiplier )

end

function Rule.Execute.dedbg( )

	-- Debug helper: prints vendor, aux market, and DE value for the current item.
	-- Use rule formula:  dedbg()
	-- Always returns false so it never categorizes anything.

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local name       = GetItemInfo( Rule.Item.h ) or "?"
	local vendor_per = select( 11, GetItemInfo( Rule.Item.h ) )
	local vendor_price = ( vendor_per or 0 ) * ( Rule.Item.count or 1 )

	-- aux market price via TSMAPI
	local aux_price = "no TSMAPI"
	if TSMAPI and TSMAPI.GetItemValue then
		local v = TSMAPI:GetItemValue( Rule.Item.h, "AuxMarket" )
		aux_price = v and ( v .. "c" ) or "nil"
	end

	-- DE value via TSM:GetDisenchantValue
	local de_str  = "TSM not found"
	local TSM_core = GetTSMAddon()
	if TSM_core then
		local de_source = ( TSM_core.db and TSM_core.db.profile and TSM_core.db.profile.destroyValueSource ) or "?"
		if type( TSM_core.GetDisenchantValue ) == "function" then
			local ok, val = pcall( TSM_core.GetDisenchantValue, TSM_core, Rule.Item.h )
			if ok then
				de_str = ( val or "nil" ) .. "c(src=" .. de_source .. ")"
			else
				de_str = "err(" .. tostring( val ) .. ")"
			end
		else
			de_str = "no fn(src=" .. de_source .. ")"
		end
	end

	DEFAULT_CHAT_FRAME:AddMessage( string.format(
		"|cffffff00[ArkInv DBG]|r %s  vend=%dc  aux=%s  de=%s",
		tostring( name ),
		vendor_price,
		tostring( aux_price ),
		de_str
...
function Rule.Execute.deovervendor( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "deovervendor"
	...
	local margin = arg1 or 0  -- default: DE simply exceeds vendor
	...
	-- get disenchant value by calling TSM:GetDisenchantValue directly
	-- (same path the tooltip uses — avoids TSMAPI:GetItemValue's GetSafeItemInfo cache miss)
	local TSM_core = GetTSMAddon()
	if not TSM_core or type( TSM_core.GetDisenchantValue ) ~= "function" then
		return false
	end
	local ok, de_price = pcall( TSM_core.GetDisenchantValue, TSM_core, Rule.Item.h )
	if not ok or not de_price or de_price == 0 then
		return false
	end

	return de_price > ( vendor_price + margin )

end

function Rule.Execute.auxprice( opt, t )
	...
	-- Try TSM unified API first (works if TradeSkillMaster + AuxTSMBridge are loaded)
	if TSMAPI and TSMAPI.GetItemValue then
		price = TSMAPI:GetItemValue( Rule.Item.h, "AuxMarket" )
	end

	-- Fallback: try direct aux global data
...
function Rule.Execute.auxpriceunder( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "auxpriceunder"
	...
	return Rule.Execute.auxprice( 1, arg1 )

end

function Rule.Execute.auxpriceover( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "auxpriceover"
	...
	return Rule.Execute.auxprice( 0, arg1 )

end

function Rule.Execute.tsmprice( opt, source, t )
	...
	local price = TSMAPI:GetItemValue( Rule.Item.h, source )
...
function Rule.Execute.tsmpriceunder( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "tsmpriceunder"
	...
	if type( arg1 ) ~= "string" then
		error( string.format( ArkInventory.Localise["RULE_FAILED_ARGUMENT_IS_INVALID"], fn, 1, ArkInventory.Localise["STRING"] ), 0 )
	end

	if not arg2 then
		error( string.format( ArkInventory.Localise["RULE_FAILED_ARGUMENT_IS_NIL"], fn, 2 ), 0 )
	end
	...
	return Rule.Execute.tsmprice( 1, arg1, arg2 )

end

function Rule.Execute.tsmpriceover( ... )

	if not Rule.Item.h or Rule.Item.class ~= "item" then
		return false
	end

	local fn = "tsmpriceover"
	...
	return Rule.Execute.tsmprice( 0, arg1, arg2 )
...
Rule.Environment = {
	...
	-- disenchant price functions (requires TradeSkillMaster + TSM_Destroying)
	deovervendor = Rule.Execute.deovervendor,
	dov = Rule.Execute.deovervendor,
	dedbg = Rule.Execute.dedbg,

	-- aux price functions (requires Aux-addon; AuxTSMBridge recommended for best results)
	auxovervendor = Rule.Execute.auxovervendor,
	aov = Rule.Execute.auxovervendor,
	auxpriceunder = Rule.Execute.auxpriceunder,
	auxpriceover = Rule.Execute.auxpriceover,
	apu = Rule.Execute.auxpriceunder,
	apo = Rule.Execute.auxpriceover,

	-- tsm price functions (requires TradeSkillMaster)
	tsmpriceunder = Rule.Execute.tsmpriceunder,
	tsmpriceover = Rule.Execute.tsmpriceover,
	tpu = Rule.Execute.tsmpriceunder,
	tpo = Rule.Execute.tsmpriceover,
