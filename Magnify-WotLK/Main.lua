...
-- Navigate the worldmap on a click.  The game's WorldMapButton_OnClick uses
-- its own internal cursor-position state which does not account for Magnify's
-- scroll frame, so it consistently picks the wrong zone at continent level.
-- We implement the continent→zone (and world→continent) cases ourselves using
-- WorldMapFrame.areaName (correctly set by UpdateMapHighlight in OnUpdate).
-- All other cases (zone-level clicks, right-click zoom-out, etc.) fall through
-- to the original game function.
-- Blizzard_BattlefieldMinimap.lua handles WORLD_MAP_UPDATE and immediately calls
-- SetMapToCurrentZone(), resetting the map back to the player's current zone.
-- This fires after SetMapZoom returns (events are processed after the current Lua
-- stack completes) but before the next OnUpdate tick.
--
-- Fix: wrap SetMapToCurrentZone so that calls triggered by WORLD_MAP_UPDATE are
-- suppressed for the one event cycle immediately following a Magnify navigation.
-- The suppression flag is cleared in the very next OnUpdate, so all subsequent
-- legitimate SetMapToCurrentZone calls (zone changes, etc.) work normally.
local Magnify_SuppressMapReset = false
do
    local _origSetMapToCurrentZone = SetMapToCurrentZone
    SetMapToCurrentZone = function(...)
        if Magnify_SuppressMapReset then return end
        return _origSetMapToCurrentZone(...)
    end
end

-- Clear the suppression flag via WORLD_MAP_UPDATE rather than OnUpdate.
-- WoW 3.3.5 runs OnUpdate *before* dispatching queued game events, so an
-- OnUpdate-based clear fires too early and lets BattlefieldMinimap through.
-- By listening to WORLD_MAP_UPDATE here (Magnify loads after Blizzard addons,
-- so our handler fires *after* BattlefieldMinimap's), the sequence is:
--   1. BattlefieldMinimap WORLD_MAP_UPDATE handler → SetMapToCurrentZone → suppressed
--   2. Our WORLD_MAP_UPDATE handler → flag cleared
local Magnify_SuppressResetClearFrame = CreateFrame("Frame")
Magnify_SuppressResetClearFrame:RegisterEvent("WORLD_MAP_UPDATE")
Magnify_SuppressResetClearFrame:SetScript("OnEvent", function()
    Magnify_SuppressMapReset = false
end)

local function MagnifyNavigateMap(button)
    local continent = GetCurrentMapContinent()
    local zone = GetCurrentMapZone()
    local areaName = WorldMapFrame.areaName

    if button == "LeftButton" then
        if continent > 0 and zone == 0 and areaName then
            -- Continent level → navigate to highlighted zone
            local zones = {GetMapZones(continent)}
            for i, name in ipairs(zones) do
                if name == areaName then
                    Magnify_SuppressMapReset = true
                    SetMapZoom(continent, i)
                    return
                end
            end
        elseif continent == 0 and areaName then
            -- World level → navigate to highlighted continent
    ...
    -- Zone-level left-click (dungeon floors, sub-zones), right-click zoom-out,
    -- or any area not found in the zone/continent list: use the original handler.
...
function Magnify.WorldMapButton_OnMouseUp()
    ...
    if not WorldMapScrollFrame.moved then
        MagnifyNavigateMap(arg1)

        -- Only reset zoom/scroll when actually zoomed in.
...
function Magnify.OnFirstLoad()
    ...
    -- Prevent the game's OnClick event from firing WorldMapButton_OnClick a second time.
    -- OnMouseUp already calls it (with the panning check), so a duplicate OnClick would
    -- navigate away from the zone that was just selected (e.g. always landing on the
    -- player's current zone instead of the one that was clicked).
    WorldMapButton:SetScript("OnClick", function() end)
