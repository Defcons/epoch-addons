...
ItemRack.CheckButtonLabels = {
	...
	["ItemRackOptSetsNoBGCheckButtonText"] = "Disable in BG/Arena",
}
...
function ItemRackOpt.ValidateSetButtons()
	...
	ItemRackOptSetsNoBGCheckButton:Disable()
	ItemRackOptSetsNoBGCheckButtonText:SetTextColor(.5,.5,.5,1)
	ItemRackOptSetsNoBGCheckButton:SetChecked(0)
	ItemRackOpt.TriStateCheckSetState(ItemRackOptShowHelm,nil)
	ItemRackOpt.TriStateCheckSetState(ItemRackOptShowCloak,nil)
	...
	if ItemRackUser.Sets[setname] then
		...
		ItemRackOptSetsNoBGCheckButton:Enable()
		ItemRackOptSetsNoBGCheckButtonText:SetTextColor(1,1,1,1)
		ItemRackOptSetsNoBGCheckButton:SetChecked(ItemRackUser.Sets[setname].NoBG and 1 or 0)
...
function ItemRackOpt.NoBGSet()
	local setname = ItemRackOptSetsName:GetText()
	if setname and ItemRackUser.Sets[setname] then
		if ItemRackOptSetsNoBGCheckButton:GetChecked() then
			ItemRackUser.Sets[setname].NoBG = 1
		else
			ItemRackUser.Sets[setname].NoBG = nil
		end
	end
end
...
function ItemRackOpt.PopulateSortList(slot)
	...
	ItemRackOpt.AddToSortList(sortList,0) -- always ensure "stop queue here" is available
