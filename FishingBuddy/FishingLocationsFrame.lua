...
local function FishiesChanged()
   local fishcount = #FishingBuddy.SortedFishies; -- Claude: replace deprecated table.getn()
   local line = 1;
   ...
   for i=1,fishcount,1 do
      ...
      for j=1,#locsort,1 do -- Claude: replace deprecated table.getn()
...
local function BothLocationsChanged()
   ...
   local line = 1;
   local zonecount = #sorted; -- Claude: replace deprecated table.getn()
   for i=1,zonecount,1 do
      ...
      if ( zidx ) then
         ...
         local subcount = #subsorted; -- Claude: replace deprecated table.getn()
         for s=1,subcount,1 do
            ...
            if ( total > 0 ) then
               ...
               if ( fh[where] ) then
                  ...
                  for j=1,#fishsort,1 do -- Claude: replace deprecated table.getn()
...
local function SubZonesChanged()
   ...
   local ff = FishingBuddy_Info["Fishies"];
   local line = 1;
   local zonecount = #FishingBuddy.SortedSubZones; -- Claude: replace deprecated table.getn()
   for i=1,zonecount,1 do
...
local function LinesChanged()
   ...
   for i=LocationLastLine,#LocationLines do -- Claude: replace deprecated table.getn()
      LocationLines[i] = 0;
   end
...
local function SetSelectedLocLine(id, line)
   ...
   if ( info ~= 0 ) then
      ...
      if ( c == 1 ) then
         ...
         FishingLocationHighlightFrame:SetPoint ( "TOPLEFT" ,  _G["FishingLocations"..id]:GetName() , "TOPLEFT" , 5 , 0 ) -- Claude: replace deprecated getglobal()
...
local function UpdateLocLine(id, line, leveloffset, c, e, text, texture)
   local locButton = _G["FishingLocations"..id]; -- Claude: replace deprecated getglobal()
   ...
   local textfield = _G["FishingLocations"..id.."NormalText"]; -- Claude: replace deprecated getglobal()
...
FishingBuddy.Locations.Update = function(self, forced)
   ...
   if ( LocationLines ) then
      ...
      local numHeaders = 0;
      local notExpanded = 0;
      for j=1,#LocationLines,1 do -- Claude: replace deprecated table.getn()
         local info = LocationLines[j];
...
local function FishOptionsInitialize()
   local menu = _G["FishingBuddyLocationsMenu"]; -- Claude: replace deprecated getglobal()
   if ( menu.fishid ) then
...
function FishingLocationsCollapseAllButton_OnClick()
   ...
   for j=1,#LocationLines do -- Claude: replace deprecated table.getn()
      local check = LocationLines[j];
      if ( check ~= 0 ) then
