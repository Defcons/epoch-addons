...
local function AddZoneIndex(zone, subzone, marker)
   ...
   if ( not zidx ) then
      ...
      zidx = #FishingBuddy_Info["ZoneIndex"]; -- Claude: replace deprecated table.getn()
...
local function AddFishie(color, id, name, zone, subzone, texture, quantity, quality, level, it, st)
   ...
   if ( id and name and FishingBuddy_Info["Fishies"][id] and not FishingBuddy_Info["Fishies"][id][CurLoc] ) then -- Claude: nil guard: id may be nil, table may not exist
      FishingBuddy_Info["Fishies"][id][CurLoc] = name;
   end
...
local function FindNextLure(b, state)
   local n = #lureinventory; -- Claude: replace deprecated table.getn()
...
-- drink first, then apply a lure if we need to
   ...
   if ( GSB("EasyLures") ) then
      ...
      if ( DoEscaped ) then
         ...
         -- is nothing better
         if ( #lureinventory > 0 ) then -- Claude: replace deprecated table.getn()
            if ( LastLure and hmhe ) then
               local b = LastLure.b;
...
local function SafeHookMethod(object, method, newmethod)
   ...
   if ( oldValue ~= _G[newmethod] ) then -- Claude: replace deprecated getglobal()
...
local function UIError_AddMessage( o, msg, a, r, g, b, hold )
   if ( msg and FL:IsFishingPole() and FishingBuddy.GetSettingBool("EasyLures") ) then
      ...
      -- put on the biggest lure we can find
         LureState = #lureinventory; -- Claude: replace deprecated table.getn()
         DoLure = lureinventory[LureState];
      else
...
FishingBuddy.UnregisterEvent = function(event, handler)
   ...
   if ( event_handlers[event] and #event_handlers[event] == 0 ) then -- Claude: replace deprecated table.getn()
...
FishingBuddy.ToggleDropDownMenu = function(level, value, menu, anchor, xOffset, yOffset)
   ...
   local frame = _G["DropDownList"..level]; -- Claude: replace deprecated getglobal()
...
FishingBuddy.EnglishList = function(list, conjunction)
   if ( list ) then
      local n = #list; -- Claude: replace deprecated table.getn()
      local str = "";
      for idx=1,n do
         local name = list[idx];
         if ( idx == 1 ) then
