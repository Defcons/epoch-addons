...
local function AddFishingSchool(kind, fishid, zidx, x, y)
   ...
   if ( not FishingBuddy_Info["FishSchools"][zidx] ) then
      ...
      if not C then return; end -- Claude: guard nil position (unmapped zone/loading)
      for _,hole in pairs(FishingBuddy_Info["FishSchools"][zidx]) do
         local d,_,_ = Astrolabe:ComputeDistance(C, Z, x, y, C, Z, hole.x, hole.y); -- Claude: fix x,x typo to x,y
         ...
         for _,hole in pairs(holes) do
            if ( hole.fish ) then
               for f,c in pairs(hole.fish) do
                  for i=1,c do -- Claude: fix crash: pairs() takes a table, not two numbers
