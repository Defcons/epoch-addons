...
local function PanelTemplates_TabResize_Copy(tab, padding, absoluteSize, maxWidth, absoluteTextSize)
	...
	local buttonMiddle = _G[tabName.."Middle"]; -- Claude: replace deprecated getglobal()
	...
	local highlightTexture = _G[tabName.."HighlightTexture"]; -- Claude: replace deprecated getglobal()
...
WatchEvents[FBConstants.FISHING_DISABLED_EVT] = function(started)
   if ( started ) then -- Claude: nil guard: started may be nil if event fires out of order
...
local function SetEntry(index, text)
   ...
   entry = _G[name]; -- Claude: replace deprecated getglobal()
   if ( not entry ) then
...
local function WatchUpdate(justTime)
   ...
   if ( noshow ) then
      ...
      for i=1, MAX_FISHINGWATCH_LINES, 1 do
         local line = _G["FishingWatchLine"..i]; -- Claude: replace deprecated getglobal()
         line:Hide();
      end
...
-- we need to be smarter about the things we filter (trash, coins, etc.)
   if ( false ) then
      local menu = _G["FishingBuddyWatcherMenu"]; -- Claude: replace deprecated getglobal()
