...
local function CollectContestPools()
   ...
   if ( CurrentContest ) then
      ...
      for idx=1,#schools do -- Claude: replace deprecated table.getn()
         local t = schools[idx];
         if ( t.kind == kind ) then
            local info = {};
            info.x = t.x;
...
local function GetNearestTen()
   if ( IsContestZone() ) then
      ...
      if not C then return contestpools, nil, nil, false; end -- Claude: guard nil position
      if ( lx ) then
...
FishingBuddy.Commands["mark"].func =
   function(what, ...)
      ...
      elseif ( what == "test" ) then
         local C, Z, x, y = Astrolabe:GetCurrentPlayerPosition();
         if not C then return; end -- Claude: guard nil position
