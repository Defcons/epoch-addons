...
-- Toggle tracking started via /db track rares or /db track chests
-- Anchored inside WorldMapPositioningGuide (inner content area), top-right
-- ============================================================================
local function CreateTrackingToggleButtons()
    ...
    -- Generic factory: creates one tiny toggle button for a given tracking list
    local function MakeToggleButton(name, trackKey, yOffset)
        ...
        -- Plain backdrop instead of UIPanelButtonTemplate so we control size/font fully
        btn:SetBackdrop({
        ...
        btn:SetScript("OnClick", function()
            if pfQuest_track and pfQuest_track[trackKey] then
                ...
                -- Currently OFF → turn on (no level range, same as plain /db track <key>)
        ...
        UpdateText()
        return UpdateText  -- return the refresh callback
    end

    -- Rares: below the title/control bar; Chests: flush below it
    local refreshRares  = MakeToggleButton("Rares",  "rares",  Y_TOP)
    local refreshChests = MakeToggleButton("Chests", "chests", Y_TOP - BTN_H)

    -- Keep both buttons in sync whenever the map opens or nodes update.
    -- WORLD_MAP_UPDATE fires whenever the map is shown/refreshed, safely outside the protected chain.
    local syncFrame = CreateFrame("Frame")
    syncFrame:RegisterEvent("WORLD_MAP_UPDATE")
    syncFrame:SetScript("OnEvent", function()
        -- Re-apply anchor in case guide repositioned (windowed <-> fullscreen toggle)
